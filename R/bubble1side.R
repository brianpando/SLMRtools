...
bubble1side <- function(xvals, yvals, xgeo = 0, ygeo = 0,
  ...
  #Create axis
  xaxis <- data.frame(x_ax = c(0,xlimit+1), y_ax = c(0,0) )
  ...
  theme.list <-
    list(
      ...
      annotate("text", x = xlimit + 1*epsilon, y = ygeo - 5*epsilon , label = xlab, size = 1.2*textsize),
      annotate("text", x = xgeo, y = ylimit + 4*epsilon, label = ylab, size = 1.2*textsize),
      #xlim(0+ 1*epsilon, xlimit + 1*epsilon), #Add limits to make it square
      ylim(0 - 7*epsilon, ylimit + 7*epsilon)  #Add limits to make it square
    )
  #Add ticks programatically
  ticks_x <- round(seq(0, xlimit+1, length.out = ticks+1),2)
  print(ticks_x)
  ticks_y <- seq(0, length(ylabels) )
  #Add ticks of x axis
  nlist <- length(theme.list)
  xlabels_full = c( xlabels )
  for (k in 1:ticks){
